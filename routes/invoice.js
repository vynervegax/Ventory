...
router.get('/new', async (req, res) => {
  ...
  {
    res.send("Please select Company on top right button besides log out button")
  }
...
router.get('/edit', async (req, res) => {
  ...
  parties = await Party.findByCompanyId(req.session.companyId)
  items = await Item.findByCompanyId(req.session.companyId)
  res.render('invoice/edit_invoice', {title: "Edit Invoice",userName: req.session.username, gstin:gstin,
                            companyName:req.session.companyName, parties:parties, 
                            items:items, invoice_page: true});
});
...
router.post('/edit/:id', async (req,res) =>{
    ...
    try{
        const data = await Invoice.updateinvoice(_id,req.body)
...
router.get('/delete/:id', async (req,res) =>{
    try{
        ...
        const data = await Invoice.deleteinvoice(req.params.id)
        //console.log(data,req.body)
    }
    catch(e){
        console.log(e)
  ...
  try{
      ...
      for(const y in invoices){
        invoice = invoices[y]
          ...
          //x = "<td><div class=\"dropdown-content\"><a id=\"navbarDropdownMenuLink"+invoice._id+"\" href=\"#\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\" class=\"nav-link tasks-toggle\"><i class=\"icon-new-file\"></i></a><div aria-labelledby=\"navbarDropdownMenuLink"+invoice._id+"\" class=\"invoice_datatable dropdown-menu tasks-list\"> </a><a data-href=\"/invoice/delete/"+invoice._id+"\" data-toggle=\"modal\" data-target=\"#confirm-delete\" class=\"dropdown-item\"><div class=\"text d-flex justify-content-between\"><strong><i class=\"fa fa-trash-o fa-fw\"></i>  Delete</strong></div></a></div></div></td>"
          ...
          x = "<td><div class=\"dropdown-content\"><a id=\"navbarDropdownMenuLink"+invoice._id+"\" href=\"#\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\" class=\"nav-link tasks-toggle\"><i class=\"icon-new-file\"></i></a><div aria-labelledby=\"navbarDropdownMenuLink"+invoice._id+"\" class=\"invoice_datatable dropdown-menu tasks-list\"><a     data-href=\"/invoice/delete/"+invoice._id+"\" data-toggle=\"modal\" data-target=\"#confirm-delete\" class=\"dropdown-item\"><div class=\"text d-flex justify-content-between\"><strong><i class=\"fa fa-trash-o fa-fw\"></i>  Delete</strong></div></a><a href=\"/invoice/print/"+invoice._id+"\" class=\"dropdown-item\"><div class=\"text d-flex justify-content-between\"><strong><i class=\"fa fa-file-pdf-o fa fw\" style=\"margin-right: 6px;padding-left: 3px;\"></i>  Print</strong></div></a></div></div></td>"
...
function RsPaise(n){
  ...
  if(nums.length == 2)
  {
    ...
    if(whole!='' && fraction==''){op='Ringgit ' + whole + ' only';} 
    if(whole!='' && fraction!=''){op='Ringgits ' + whole + 'and paise ' + fraction + ' only';}
